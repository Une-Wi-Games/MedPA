 ...
 */
 ...
 /*:
 * @plugindesc Дополнительный персонаж в меню
 * @author Sasuke KANNAZUKI (thanks to Yoji Ojima)
 *
 * @param subMemberIdVal1
 * @desc Номер перменной где хранится номер первого дополнительного персонажа.
 * @default 11
 *
 * @param subMemberIdVal2
 * @desc Номер перменной где хранится номер второго дополнительного персонажа.
 * @default 12
 *
 * @param subMemberIdVal3
 * @desc Номер перменной где хранится номер третьего дополнительного персонажа.
 * @default 13
 *
 * @param subMemberIdVal4
 * @desc Номер перменной где хранится номер четвёртого дополнительного персонажа.
 * @default 14
 * 
 * @param displayIfNone
 * @desc Если дополнительных персонажей нет, показаывать ли окно всё равно?
 * (1=да, 0=нет)
 * @default 0
 * 
 * @param subMemberText
 * @desc Заголовок окна с дополнительными персонажами.
 * @default Доп.персонажи
 *
 * @param subMemberNoneText
 * @desc Этот текст будет отображаться, если нет дополнительных персонажей
 * @default Пусто
 *
 *
 * @help Этот плагин не содержит никаких команд.
 * 
 * Дополнительные персонажи отображаются в меню под основными командами.
 * Дополнительный персонаж не участвует в битве и не отображается на карте!
 * Например можно использовать чтобы запомнить кто последний дал задание или с кем нужно поговорить.
 * 
 * Если дополньный персонаж не нужен, установите значение переменной или параметра в 0.
 *
 * !Внимание: этот плагин работает только со стандартным меню.
 *  С другими плагинами, которые менют меню (например AltMenuScreen.js), работать не будет.
 * 
 */

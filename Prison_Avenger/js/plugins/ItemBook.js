 ...
 */
 ...
 /*:ru
 * @plugindesc Книга предметов - подробное описание предметов.
 * @author Yoji Ojima
 *
 * @param Unknown Data
 * @desc  Так будут показаны ещё не открытые предметы.
 * @default ??????
 *
 * @param Price Text
 * @desc Заголовок для цены предмета.
 * @default Цена
 *
 * @param Equip Text
 * @desc Заголовок для слота экипировки.
 * @default Слот
 *
 * @param Type Text
 * @desc Заголовок для типа предмета.
 * @default Тип
 *
 * @help
 *
 * В книгу изначально добавляются все предметы (предметы, оружие и броня) из базы. 
 * Команды: (В событии "Команда доп. модуля", 3 страница)
 *   ItemBook open            # Открыть книгу.
 *   ItemBook add weapon 3    # Добавить оружие с номером 3 в книгу (открыть информацию)
 *   ItemBook add armor 4     # Добавить броню с номером 4 в книгу (открыть информацию)
 *   ItemBook remove armor 5  # Убрать броню номер 5 из книги (закрыть информацию)
 *   ItemBook remove item 6   # Убрать предмет с номером 6 из книги (закрыть информацию)
 *   ItemBook complete        # Открыть информацию о всех предметах.
 *   ItemBook clear           # Закрыть информацию о всех предметах.
 *
 * Дополнительно в "Примечание" оружия, предмета или брони.
 *   <book:no>                # Этот предмет не появится в книге.
 */

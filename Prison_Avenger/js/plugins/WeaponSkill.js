 ...
 */
 ...
 /*:ru
 * @plugindesc Специальная атака для оружия.
 * @author Sasuke KANNAZUKI
 *
 * @help Этот плагин не содержит никаких команд.
 *
 * По умолчанию всё оружие в игре при атаке использует навык номер 1 из базы данных.
 * Данный плагин позволяет изменить номер навыка, который будет использовать то или иное оружие при атаке.
 * Пишем в "Примечание" оружия.
 * <skill_id:3> - теперь оружие при атаке будет использовать навык под номером 3.
 * Если ничего не писать, то по умолчанию будет 1 навык "Атака".
 *
 * Обратите внимение:
 * - Если экипировано два оружия, то используется навык первого оружия.
 * - It is most favorable for "skill type" to be "none"(=0),
 *  otherwise you cannot attack when your skill is blocked.
 * (Прим.перевода: тут не понял, но вроде если навык нельзя использовать (например нет маны), то атака в бою будет недоступна).
 *
 * Для чего нужен этот плагин:
 * - например сделать оружие, которое атакует всех врагов сразу
 * - сделать оружие, которое имеет две или три атаки сразу
 * - сделать оружие, которое лечит союзника и проч.
 */

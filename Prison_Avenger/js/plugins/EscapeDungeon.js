 ...
 */
 /*:ru
 * @plugindesc Телепорт в указанную позицию из подземелья.
 * @author Sasuke KANNAZUKI
 *
 * @param mapIdVal
 * @desc Номер переменной, в которой хранится номер карты (ID). На эту карту будет телепорт.
 * Если указать 0, то телепорт запрещён (не случится).
 * @default 21
 * 
 * @param xCoordVal
 * @desc Номер переменной, в которой хранится координата по X - куда будет телепортирован персонаж.
 * @default 22
 *
 * @param yCoordVal
 * @desc Номер переменной, в которой хранится координата по Y - куда будет телепортирован персонаж.
 * @default 23
 *
 * @help Этот плагин не содержит никаких команд.
 *
 * - Задайте значения в переменных, когда игрок заходит в подземелье.
 * - Будьте уверены, что отключили телепорт, когда игрок покинул подземелье,
 *  например когда игрок переходит на другую карту.
 *   (чтобы отключить, нужно в переменной, которая задана в mapIdVal, установить значение 0).
 * - Если игрок использовал телепорт, то он отключится автоматически.
 *
 * Необходимые параметры предмета \ заклинания:
 * - Область - Нет.
 * - Случай - Экран меню. (В битве нельзя использовать).
 * - В "Примечания" пишем: <ESCAPE>
 */

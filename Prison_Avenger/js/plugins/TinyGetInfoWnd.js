 ...
 */
 /*:ru
 * @plugindesc Сообщение когда игрок получает или теряет предмет.
 * @author Sasuke KANNAZUKI
 *
 * @param Event Command Switch
 * @desc Если переключатель под этим номером включён, то сообщение будет появляется автоматически,
 * когда партия получает или теряет предметы.
 *  - Работает только на карте, в событиях. (Изменить оружие, броню, предмет).
 * @default 22
 * 
 * @param Y position type
 * @desc Положение окна. 0:вверху 1:внизу
 * @default 1
 * 
 * @param textGainItem
 * @desc Заголовок сообщения о получении предмета.
 *  - если указать %1, то заместо будет подставлен тип предмета (оружие/броня/предмет).
 * @default Вы получили %1
 * 
 * @param textLoseItem
 * @desc Заголовок сообщения о потере предмета
 * - если указать %1, то заместо будет подставлен тип предмета (оружие/броня/предмет).
 * @default Вы потеряли %1
 *
 * @param SE filename
 * @desc Имя звукового SE файла, который будет воспроизводиться при получении предмета.
 * note: Когда вы теряете предмет, звук не воспроизводиться.
 * @default Equip1
 * 
 * @param SE volume
 ...
 * TinyGetInfoWnd arg0 arg1 arg2 arg3
 * arg0 - должен быть: 'item', 'weapon', или 'armor'. (предмет, оружие, броня).
 * arg1 - должен быть номер (ID) предмета(или экипировки),
 *  или 'V' + номер(напр. 'V20') где номер - это номер переменной, в которой номер ID предмета.
 * arg2 - должен быть 'gain' или 'lose'. (по умолчанию 'gain'). (дать, потерять)
 * arg3 - количество. (по умолчанию 1).
 *  (аналогично arg1, можно указать номер переменной, в котором количество (напр. 'V15'))
 * Примеры:
 * TinyGetInfoWnd weapon 14 gain 2  # Сообщение - игрок получил оружие номер 14, 2 штуки. 
 * TinyGetInfoWnd armor 20 lose 1   # Сообщение - игрок потерял броню номер 20, 1 штуку.
 *   (если броня была экипирована, она всё равно удалится)
 * TinyGetInfoWnd item 7    # Сообщение - игрок получил предмет под номером 7, 1 штуку.
 *   (в этом примере, по умолчанию, arg2='gain' и arg3='1')
 * TinyGetInfoWnd item V10 gain 3   # Сообщение - игрок получил 3 предмета (номер которых хранится в переменной под номером 10).
 * 
 * Дополнительно в "Примечание" предмета.
 * <info:текст> : Этот текст будет показан в сообщении, как объяснение. Если не указывать, то отображается 
 * стандартное описание предмета из базы данных.
 *
 * К сведению:
 * Плагин не отнимит больше предметов, чем есть у группы.
 * Когда у вас есть 3 предмета, а вы выполните команду 'lose 5' (потерять 5), то в сообщении будет написано, что потеряно 3. 
 * Если у партии нет совсем этого предмета, то сообщение не отобразится.
 */

 ...
 */
 ...
 /*:ru
 * @plugindesc Специальный тип экипировки для персонажа.
 * @author Sasuke KANNAZUKI
 * @help
 * Команды: (В событии "Команда доп. модуля", 3 страница)
 * ExtraEquipType arg0 arg1 arg2 arg3
 * arg0 - 'add' или 'remove'. Первое - добавить, второе - убрать.
 * arg1 - номер персонажа в базе данных.
 * arg2 - 'weapon' или 'armor'. Первое - оружие, второе - броня.
 * arg3 - номер типа брони или оружия. Не путать с номером оружия или брони. Номер типа смотреть в "Типы".
 * Пример:
 * ExtraEquipType add 1 weapon 7  - Персонаж 1 (Гарольд), может теперь экипировать лук (7 тип).
 * ExtraEquipType remove 2 armor 3 - Персонаж 2 (Тереза), больше не может носить "Световая броня" (тип 3).
 * - если на момент удаления, предмет был экипирован, он автоматически сниметься с персонажа.
 * Примечание!:
 * Нельзя запретить персонажу экипировать оружие или броню того типа, который был задан ему изначально в базе * данных (в редакторе).
 * 
 */

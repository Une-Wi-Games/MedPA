 ...
 */
 ...
 /*:ru
 * @plugindesc Изображение поверх карты (foreground)
 * @author Sasuke KANNAZUKI (thanks to Yoji Ojima)
 * 
 * @help Этот плагин не содержит никаких команд.
 *
 * 'foreground' - это как фон смещения на карте, только над всей картой, а не за ней.
 * изображение должно быть в папке img/parallaxes.
 * С этим плагином можно реализовать облака и туман на карте.
 *
 * "Примечание" для карты (правой кнопкой по карте - редактировать):.
 *  <fgName:имя_файла>     имя изображения в папке img/parallaxes. 
 *    если имя начинается с '!', то будет понято как "foreground zero". (Прим. перевода - тут я не понял).
 *  <fgLoopX:число>  число - зациклить по оси Х. 0:нет 1:да
 *    если не указать, то будет 0 по умолчанию.
 *  <fgLoopY:число>  число - зациклить по оси Y. 0:нет 1:да
 *    если не указать, то будет 0 по умолчанию.
 *  <fgSx:число>     число - скорость вращения по Х.
 *    если зацикливание по Х не указано (0), то эта настройка игнорируется.
 *    если не указать, то будет 0 по умолчанию.
 *  <fgSy:число>     число - скорость вращения по  Y.
 *    если зацикливание по Y не указано (0), то эта настройка игнорируется.
 *    если не указать, то будет 0 по умолчанию.
 * 
 * Примеры:
 * <fgName:sample1><fgLoopX:1><fgLoopY:1>
